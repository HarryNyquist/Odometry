...
def trim_time_array(original_array, new_array):
    ...
    #new_array[:idx]
    if len(new_array[idx:])%2!=0:
        idx += 1
    return (idx, new_array[idx:])

def trapezoidal_rule(y,x): #y = f(x)
    integral = np.zeros_like(y)

    for i in range(1,len(x)):
        trap_height = x[i] - x[i-1]
        trap_parallel_sides = (y[i] + y[i-1])/2 
        integral[i] = integral[i-1] + trap_height * trap_parallel_sides
    return integral 
